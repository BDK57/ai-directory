  ...
  Trending: [
    {
      ...
      rating: "4.7 Rating",
      description: "Lorem ipsum dolor sit amet consectetur. Id eu ipsum urna sed consectetur aliquam scelerisque.",
...
<div className="text-white mt-16 flex justify-between items-center px-16">
  ...
  <p className="mt-4 max-w-4xl default-paragraph  z-20">
    ...
    Aliquam mauris ullamcorper sed amet curabitur bibendum eu pharellus ullamcorper.
  ...
  <div className="mt-6">
    <CustomButtonWithIcon text={"Explore More"} />
      ...
      {menuItems.map((item, index) => {
          // Left margin increases as we move down the list
          const leftSpacing = index * 8; // Adjust this value for spacing

          return (
            <li
            key={index}
            className={`transition px-4 py-2 rounded-lg flex items-center gap-4 cursor-pointer font-montserrat
              ${index === activeIndextab ? "text-white font-semibold  px-6" : "text-gray-400"}
              ${index !== activeIndextab ? "opacity-70 hover:opacity-100" : ""}
            `}
            style={{
                marginLeft: `${getLeftSpacing(index)}px`,
                background: index === activeIndextab 
          ...
          >
            {item} {index === activeIndextab && <span className="">✍️</span>}
          </li>
          );
        })}
      </ul>
  ...
  <div className="flex w-full justify-between mt-4">
        ...
        {/* <div className="absolute -bottom-12 right-4 flex gap-2">
          <button
            ref={prevRef}
            className="bg-[#4A4588] text-white p-2 rounded-md hover:bg-[#322C68] transition"
          >
            Left
          </button>
          <button
  ...
  {slides.slice(1).map((_, index) => (
    <div
      key={index}
      style={{border: "1px solid white" ,width:"16px", height:"16px"}}
      className={` rounded-full border border-white transition-all duration-300 ${
        activeIndex === index ? "bg-[#9747FF]  " : "bg-transparent"
      }`}
...
<path d="M9.54801 6.57999L8.48701 5.51999L2.70801 11.297C2.61486 11.3896 2.54093 11.4996 2.49048 11.6209C2.44003 11.7421 2.41406 11.8722 2.41406 12.0035C2.41406 12.1348 2.44003 12.2648 2.49048 12.3861C2.54093 12.5073 2.61486 12.6174 2.70801 12.71L8.48701 18.49L9.54701 17.43L4.12301 12.005L9.54801 6.57999Z" fill="#4A4588"/>
  ...
  <button
    ...
    className={`w-9 h-9 flex items-center justify-center rounded-md border transition-all
      ${activeIndex < slides.length - 1 ? "border-white bg-gradient-to-br from-[#D851A3] to-[#9747FF]" : "border-gray-600"}
    `}
  >
    {/* <span
      className={`border-r-2 border-b-2 w-3 h-3 transform rotate-[320deg] transition-all
        ${activeIndex < slides.length - 1 ? "border-white" : "border-gray-700"}
      `}
    ></span> */}
    <svg width="12" height="24" viewBox="0 0 12 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<div className='bg-primary-black '>
      <div className="relative w-full h-screen  px-24 w-full " >
       <div className='absolute right-0 bottom-0 w-full '>
        <img src={communityImage} alt="" className='w-full h-full object-cover' style={{ filter: "grayscale(0.9)"}} />
       </div>
...
<div className='border-x border-x-primary-white-20 container z-20 relative h-screen'>
    <div className="w-full  text-white py-10 relative z-20">
      <h1 className="sub-heading ">Yourdirectory.ai <br /> Community</h1>
        <div className='flex justify-between'>
            <div className='w-[60%] '>
      <p className="default-paragraph  mt-2">Lorem ipsum dolor sit amet consectetur. Id eu ipsum urna sed consectetur <br/>  aliquam scelerisque.</p>
      </div>
      {/* Tabs */}
      <div className="flex    z-20 cursor-pointer   mt-6 border-b border-gray-700 w-[40%]">
        {tabs.map((tab) => (
          <div
            key={tab}
            className={`pb-2 text-xl text-center font-medium w-full relative ${
              activeTab === tab ? "text-white" : "text-gray-400"
            }`}
            onClick={() => setActiveTab(tab)}
          >
            {tab}
            {activeTab === tab && <div className="h-1 w-full bg-[#9747FF] absolute -bottom-[2px] left-0" />}
          </div>
        ))}
      </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        {data[activeTab].map((item, index) => (
          <div className="bg-[#161027] p-4 rounded-xl flex items-center gap-4 w-full border border-[#FFFFFF38]">
          {/* Image Section */}
          <div className="w-[188px] h-[188px] rounded-lg overflow-hidden">
            <img
              src={communityImage} // Replace with your image path
              alt="Midjourney AI"
